...
class DataTableStream: public Stream {
  public:
    ...
    DataTableStream(const std::string& table_filepath,
        ...
        , column_count_{column_names.size()}
    ...
    std::size_t get_current_row_index() const {
        return current_row_index_;
    }
    ...
    template <typename T>
    DataTableStream* write_row(T field) {
        write_column(field);
